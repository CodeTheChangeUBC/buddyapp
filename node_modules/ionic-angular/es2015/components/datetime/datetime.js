...
export class DateTime extends Ion {
    ...
    open() {
        ...
        const /** @type {?} */ picker = this._pickerCtrl.create(pickerOptions);
        ...
        if (isPresent(template)) {
            ...
            parseTemplate(template).forEach(format => {
                ...
                if (column.options.length) {
                    ...
                    var /** @type {?} */ selected = column.options.find(opt => opt.value === getValueFromFormat(this._value, format));
                    if (selected) {
    ...
    validate(picker) {
        ...
        let /** @type {?} */ columns = picker.getColumns();
        // find the columns used
        let /** @type {?} */ yearCol = columns.find(col => col.name === 'year');
        let /** @type {?} */ monthCol = columns.find(col => col.name === 'month');
        let /** @type {?} */ dayCol = columns.find(col => col.name === 'day');
        let /** @type {?} */ yearOpt;
        let /** @type {?} */ monthOpt;
        let /** @type {?} */ dayOpt;
        ...
        let /** @type {?} */ numDaysInMonth = 31;
        let /** @type {?} */ selectedMonth;
        if (monthCol) {
        ...
        let /** @type {?} */ minCompareVal = dateDataSortValue(this._min);
        let /** @type {?} */ maxCompareVal = dateDataSortValue(this._max);
        if (monthCol) {
        ...
        if (dayCol) {
            if (isPresent(selectedMonth)) {
                ...
                for (i = 0; i < dayCol.options.length; i++) {
                    ...
                    // is within the min/max date range
                    var /** @type {?} */ compareVal = dateSortValue(selectedYear, selectedMonth, dayOpt.value);
     ...
     */
    divyColumns(picker) {
        let /** @type {?} */ pickerColumns = picker.getColumns();
        let /** @type {?} */ columns = [];
        ...
        if (columns.length === 2) {
            var /** @type {?} */ width = Math.max(columns[0], columns[1]);
            pickerColumns[0].align = 'right';
            pickerColumns[1].align = 'left';
            pickerColumns[0].optionsWidth = pickerColumns[1].optionsWidth = `${width * 17}px`;
        }
        else if (columns.length === 3) {
            var /** @type {?} */ width = Math.max(columns[0], columns[2]);
            pickerColumns[0].align = 'right';
