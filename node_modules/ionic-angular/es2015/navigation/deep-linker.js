...
export class DeepLinker {
    ...
    navChange(direction) {
        ...
        if (direction) {
            ...
            if (activeNav) {
                ...
                const /** @type {?} */ browserUrl = this._serializer.serialize(this.segments);
    ...
    getComponentFromName(componentName) {
        const /** @type {?} */ segment = this._serializer.createSegmentFromName(componentName);
    ...
    createUrl(nav, nameOrComponent, data, prepareExternalUrl = true) {
        ...
        if (segment) {
            const /** @type {?} */ path = this.pathFromNavs(nav, segment.component, data);
    ...
    pathFromNavs(nav, component, data) {
        ...
        while (nav) {
            ...
            if (isTab(nav)) {
                ...
                tabSelector = this.getTabSelector(/** @type {?} */ (nav));
                segments.push({
                    id: tabSelector,
                    name: tabSelector,
                    component: null,
    ...
    initViews(segment) {
        ...
        const /** @type {?} */ view = new ViewController(segment.component, segment.data);
        view.id = segment.id;
        views.push(view);
        return views;
    }
...
function DeepLinker_tsickle_Closure_declarations() {
     ...
     * @type {?}
     */
    DeepLinker.prototype.segments;
    /**
     * \@internal
     * @type {?}
     */
    DeepLinker.prototype.history;
    /**
     * \@internal
     * @type {?}
     */
    DeepLinker.prototype.indexAliasUrl;
    /** @type {?} */
...
export function setupDeepLinker(app, serializer, location) {
    const /** @type {?} */ deepLinker = new DeepLinker(app, serializer, location);
    deepLinker.init();
    return deepLinker;
}
/**
