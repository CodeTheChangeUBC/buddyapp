...
(function (factory) {
    ...
    var DateTime = (function (_super) {
        ...
        DateTime.prototype.open = function () {
            ...
            var /** @type {?} */ picker = this._pickerCtrl.create(pickerOptions);
            ...
            if (util_1.isPresent(template)) {
                ...
                datetime_util_1.parseTemplate(template).forEach(function (format) {
                    ...
                    if (column.options.length) {
                        ...
                        var /** @type {?} */ selected = column.options.find(function (opt) { return opt.value === datetime_util_1.getValueFromFormat(_this._value, format); });
                        if (selected) {
        ...
        DateTime.prototype.validate = function (picker) {
            ...
            var /** @type {?} */ columns = picker.getColumns();
            // find the columns used
            var /** @type {?} */ yearCol = columns.find(function (col) { return col.name === 'year'; });
            var /** @type {?} */ monthCol = columns.find(function (col) { return col.name === 'month'; });
            var /** @type {?} */ dayCol = columns.find(function (col) { return col.name === 'day'; });
            var /** @type {?} */ yearOpt;
            var /** @type {?} */ monthOpt;
            var /** @type {?} */ dayOpt;
            ...
            var /** @type {?} */ numDaysInMonth = 31;
            var /** @type {?} */ selectedMonth;
            if (monthCol) {
            ...
            if (dayCol) {
                if (util_1.isPresent(selectedMonth)) {
                    ...
                    for (i = 0; i < dayCol.options.length; i++) {
                        ...
                        // is within the min/max date range
                        var /** @type {?} */ compareVal = datetime_util_1.dateSortValue(selectedYear, selectedMonth, dayOpt.value);
        ...
        DateTime.prototype.divyColumns = function (picker) {
            var /** @type {?} */ pickerColumns = picker.getColumns();
            var /** @type {?} */ columns = [];
